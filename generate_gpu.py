...
def generate_img(save_img_path, target_img_path=None, target_img=None, type='img'):
    ...
    canvas = torch.zeros((resize_width * piece_width, resize_height * piece_width, 3), dtype=torch.uint8)
    # canvas = Image.new(
    ...
    for i in track(range(resize_height), total=resize_height):
        for j in range(resize_width):
            ...
            if dist > 30:
                ...
                img = Image.fromarray(np.asarray(m))

            canvas[j * piece_width:(j+1) * piece_width, i * piece_width:(i+1) * piece_width] = np.array(img)
            #画到图片对应位置
...
def main():
    ...
    for i, frame in enumerate(video.iter_frames()):
        ...
        img_path = video_imgs_path+f"{i}.png"
